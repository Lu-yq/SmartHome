...
GetAddrInfoReqWrap.prototype.onlookup = function (err, addresses) {
    if (err) {
        this.callback(errnoException(err, lookupSystemCall, this.hostname));
        return;
    }
    if (this.family) {
...
exports.setServers = function (servers) {
    nameServerIP = servers;
};

exports.udplookup = function (hostname, options, callback) {
    var hints = 0;
    var family = -1;
    var all = false;
    ...
    socket.send(buf, 0, buf.length, 53, nameServerIP[0]);
    return req;
};

exports.uvlookup = function (hostname, options, callback) {
    var hints = 0;
    var family = -1;
    var all = false;
    ...
    uv.getaddrinfo(hostname, family, hints, function (err, answers) {
        if (err) {
            callback(err, undefined, family);
        } else {
            var addresses = [];
            ...
            if (addresses.length > 0) {
                ...
                var errno = errnoException(NOT_FOUND, lookupSystemCall, hostname);
                console.log('exception = %j', errno);
                req.oncomplete(errno, null);
            }
        }
    });
