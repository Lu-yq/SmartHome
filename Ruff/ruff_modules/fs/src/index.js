...
fs.readdirSync = function (path) {
    ...
    while (true) {
        var item = uv.fs_scandir_next(entry);
        if (!item) {
            break;
        }
        items.push(item.name);
...
fs.write = function (fd, buffer, offset, length, position, callback) {
    ...
    return binding.write(fd, util._toNativeBuffer(buffer), position || -1, {
...
fs.writeSync = function (fd, buffer, offset, length, position) {
    ...
    buffer = util._toNativeBuffer(buffer);

    return binding.write(fd, buffer, position || -1);
};
