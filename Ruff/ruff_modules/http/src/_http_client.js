...
var net = require('net');
var url = require('url');
var HTTPParser = require('./_http_parser.js').HTTPParser;
...
var OutgoingMessage = require('./_http_outgoing.js').OutgoingMessage;
...
function ClientRequest(options, cb) {
    ...
    if (self.socketPath) {
        ...
        var newSocket = self.agent.createConnection(optionsPath, oncreate);
        if (newSocket && !called) {
            called = true;
        ...
        if (!self.agent.keepAlive && self.agent.maxSockets == Infinity) {
            self._last = true;
            self.shouldKeepAlive = false;
        } else {
            self._last = false;
