...
function Server(opts, requestListener) {
    ...
    if (process.features && process.features.tls_npn && !opts.NPNProtocols) {
        opts.NPNProtocols = ['http/1.1', 'http/1.0'];
    }

    if (process.features && process.features.tls_alpn && !opts.ALPNProtocols) {
...
function createConnection(port, host, options) {
    ...
    var socket = tls.connect(options, function () {
        ...
        // that._cacheSession(options._agentKey, socket.getSession());
    });

    // Evict session on error
    socket.once('close', function (err) {
        if (err)
            that._evictSession(options._agentKey);
    });
