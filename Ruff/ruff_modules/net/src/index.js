...
Socket.prototype.setNoDelay = function (enable) {
    ...
    if (!this._handle) {
        this.once('connect', enable ? this.setNoDelay : function () { return that.setNoDelay(enable); });
        return this;
    }
...
Socket.prototype.setKeepAlive = function (setting, msecs) {
    ...
    if (!this._handle) {
        this.once('connect', function () { return that.setKeepAlive(setting, msecs); });
        return this;
    }
...
Socket.prototype._write = function (data, encoding, cb) {
    ...
    if (encoding && typeof data === 'string') {
        data = util._toNativeBuffer(new Buffer(data, encoding));
    } else if (data instanceof Buffer) {
        data = util._toNativeBuffer(data);
...
function connect(socket, address, port, addressType, localAddress, localPort) {
    ...
    if (localAddress || localPort) {
        if (addressType === 4) {
            ...
            socket._destroy(new TypeError('Invalid addressType: ' + addressType));
            return;
        }
        ...
        try {
            socket._handle.bind(localAddress, localPort);
        } catch (err) {
    ...
    try {
        socket._handle.connect(address, port, function (error) {
            afterConnect(socket, address, port, localAddress, localPort, error);
        });
    } catch (err) {
        var sockname = socket._getsockname();
        var details;
...
function afterConnect(socket, address, port, localAddress, localPort, error) {
    // callback may come after call to destroy
    if (socket.destroyed) {
        return;
    }
    ...
    if (error) {
        ...
        if (details) {
            ex.localAddress = localAddress;
            ex.localPort = localPort;
        }
        socket._destroy(ex);
        return;
    }
...
Server.prototype._listen2 = function (address, port, addressType, backlog, fd) {
    ...
    if (this._handle) {
        ...
        if (!address && typeof fd !== 'number') {
            try {
                handle = createServerHandle('::', port, 6, fd);
                address = '::';
                addressType = 6;
            } catch (err) {
                handle = null;
        ...
        if (handle === null) {
            try {
                handle = createServerHandle(address, port, addressType, fd);
            } catch (err) {
                var error = exceptionWithHostPort(err, 'listen', address, port);
...
function listen(self, address, port, addressType, backlog, fd) {
    ...
    function cb(err, handle) {
        ...
        if (!err && port > 0 && handle.getsockname) {
            try {
                var sockname = handle.getsockname();

                if (port !== sockname.port) {
                    throw errnoException(uv.UV_EADDRINUSE, 'bind');
                }
            } catch (error) {
                err = error;
            }
        ...
        if (err) {
            err = exceptionWithHostPort(err, 'bind', address, port);
...
if (process.platform === 'win32') {
    ...
    exports._setSimultaneousAccepts = function (handle) {
        ...
        if (simultaneousAccepts === undefined) {
            simultaneousAccepts = (process.env.RUFF_MANY_ACCEPTS &&
                                                         process.env.RUFF_MANY_ACCEPTS !== '0');
        }
