...
AppRunner.prototype._addDevice = function (id) {
    this._devices[id] = mock(new EventEmitter(), true);
};
...
AppRunnerContainer.prototype._doRun = function (appPath, runnable, args) {
    ...
    var packageFilePath = path.join(appPath, 'package.json');

    if (fs.existsSync(packageFilePath) && safeRequire(packageFilePath).main) {
        appEntryFile = path.join(appPath, safeRequire(packageFilePath).main);
    } else {
        appEntryFile = path.join(appPath, 'src', 'index.js');
    }

    safeRequire(appEntryFile);
