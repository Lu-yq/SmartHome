...
TLSSocket.prototype._tlsWrite = function (data, encoding, cb) {
    var ret = mbedtls.write(this._tlsHandle, util._toNativeBuffer(data));
    if (cb) {
        cb(null);
    }
    return ret;
...
TLSSocket.prototype.push = function (chunk, encoding) {
    ...
    while (true) {
        ...
        if (ret instanceof Uint8Array) {
            push.call(this, Buffer.from(ret));
            continue;
        }
        ...
        switch (ret) {
            ...
            case mbedtls.EOF:
                this.emit('end');
                return true;
            case mbedtls.ERR_SSL_INVALID_PK:
                this.invalidPkError = new Error('no own certificate and private key');
                this.emit('_tlsError', this.invalidPkError);
                return true;
            default:
...
exports.connect = function (/* [port, host], options, cb */) {
    ...
    if (!options.socket) {
        ...
        socket.connect(connectOptions, function () {
            ...
            if (options.ca instanceof Buffer) {
                options.ca = util._toNativeBuffer(options.ca);
            }
            mbedtls.config_cacert(socket._tlsHandle, options.ca);

            if (options.cert !== undefined && options.key !== undefined) {
                mbedtls.config_owncert(socket._tlsHandle, new Buffer(options.cert));
                mbedtls.config_ownkey(socket._tlsHandle, new Buffer(options.key));
            } else {
                mbedtls.config_default_owncert(socket._tlsHandle);
                mbedtls.config_default_ownkey(socket._tlsHandle);
            }
